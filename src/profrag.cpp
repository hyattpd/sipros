...
ProteinFragmentRules::ProteinFragmentRules() {
  ...
  max_polymorphisms_ = 1;
  max_ptms_ = 1;
  parent_mass_error_ = 0.04;
  fragment_mass_error_ = 0.04;
  left_offset_ = 0.0;
  right_offset_ = 18.0105;
  fragmentation_method_ = "CID";
}
...
bool ProteinFragmentRules::populate_from_xml_config() {
  after_residues_ = ProNovoConfig::getCleavageAfterResidues(); 
  before_residues_ = ProNovoConfig::getCleavageBeforeResidues(); 
  max_missed_cleave_ = ProNovoConfig::getMaxMissedCleavages();
  min_specific_cleave_ = ProNovoConfig::getMinSpecificCleavages();
  min_peptide_length_ = 6;
  max_peptide_length_ = 60;
  test_start_removal_ = ProNovoConfig::getTestStartRemoval();
  min_peptide_mass_ = ProNovoConfig::getMinPeptideMass();
  max_peptide_mass_ = ProNovoConfig::getMaxPeptideMass();
  max_polymorphisms_ = ProNovoConfig::getMaxPolymorphismCount();
  max_ptms_ = ProNovoConfig::getMaxPTMcount();
  parent_mass_error_ = ProNovoConfig::getMassAccuracyParentIon();
  fragment_mass_error_ = ProNovoConfig::getMassAccuracyFragmentIon();
  left_offset_ = ProNovoConfig::getTerminusMassN();
  right_offset_ = ProNovoConfig::getTerminusMassC();
  fragmentation_method_ = ProNovoConfig::getFragmentationMethod();
  return true;
}
...
void ProteinFragment::increment_poly() {
  ...
  if(end != -1) {
    ...
    if(end < (int)protein.length()) { 
      mass = pr->left_offset()+pr->right_offset() + msum_map[end];
...
void ProteinFragment::increment_ptm() {
  ...
  if(ptm_loop_end != -1) {
    ptm_loop_end++;
    if(ptm_loop_end < (int)protein.length()) { 
      mass = pr->left_offset()+pr->right_offset() + msum_map[ptm_loop_end];
      if(ptm_loop_begin > 0) mass -= msum_map[ptm_loop_begin-1];
      mass += ptmlist->mass_shift(ptm_val);
      if(ptm_loop_end > ptm_pos && (ptm_loop_end == (int)(protein.length()-1) || cl_map[ptm_loop_end] == true)) right_cleave++;
  ...
  if(ptm_val != -1) {
    ...
    right_cleave = 0;
    while(ptmlist->residue(ptm_val) != '[' && ptmlist->residue(ptm_val) != ']' && 
          ptmlist->residue(ptm_val) != protein[ptm_pos] && ptm_val < ptmlist->size())
      ptm_val++;
    if(ptm_val < ptmlist->size()) { 
      if(ptmlist->residue(ptm_val) == '[' && ptm_pos != ptm_loop_begin) return;
      if(ptmlist->residue(ptm_val) == ']' && ptm_pos != ptm_loop_end) return;
      mass = pr->left_offset()+pr->right_offset() + msum_map[ptm_loop_end];
      if(ptm_loop_begin > 0) mass -= msum_map[ptm_loop_begin-1];
      mass += ptmlist->mass_shift(ptm_val);
      if(ptm_loop_begin < ptm_pos && cl_map[ptm_loop_begin] == true) left_cleave++;
...
DatabasePeptide * ProteinFragment::get_dbpeptide_poly() {
  ...
  peptide.insert(0, 1, '[');
  peptide += "]";
  return (new DatabasePeptide(dbid, begin+1, end-begin+1, peptide, 0));
}
...
DatabasePeptide * ProteinFragment::get_dbpeptide_ptm() {
  ...
  peptide.insert(0, 1, '[');
  peptide += "]";
  if(ptmlist->residue(ptm_val) == '[') {
    peptide.insert(1, 1, ptmlist->symbol(ptm_val));
  }
  else if(ptmlist->residue(ptm_val) == ']') {
    peptide += ptmlist->symbol(ptm_val);
  }
  else peptide.insert(ptm_pos - ptm_loop_begin + 2, 1, ptmlist->symbol(ptm_val));
  return (new DatabasePeptide(dbid, ptm_loop_begin+1, ptm_loop_end-ptm_loop_begin+1, peptide, 1));
}

bool ProteinFragment::is_valid_poly() {
//  if(end - begin + 1 < pr->min_peptide_length()) return false;
  if(mass < pr->min_peptide_mass() || mass > pr->max_peptide_mass()) return false;
  if(begin != 0 && (begin != 1 || pr->test_start_removal() == false) && 
       ((mutpos == begin && pr->is_cleavage_site(protein[mutpos-1], residues[mutval]) == false)
...
bool ProteinFragment::is_valid_ptm() {
  if(ptmlist->residue(ptm_val) == '[' && ptm_pos != ptm_loop_begin) return false;
  if(ptmlist->residue(ptm_val) == ']' && ptm_pos != ptm_loop_end) return false;
  if(ptmlist->residue(ptm_val) != '[' && ptmlist->residue(ptm_val) != ']' && 
     ptmlist->residue(ptm_val) != protein[ptm_pos]) return false;
  if(mass < pr->min_peptide_mass() || mass > pr->max_peptide_mass()) return false;
//  if(ptm_loop_end - ptm_loop_begin + 1 < pr->min_peptide_length()) return false;
  if(ptm_loop_begin != 0 && (ptm_loop_begin != 1 || pr->test_start_removal() == false) &&
       ((ptm_pos == ptm_loop_begin && pr->is_cleavage_site(protein[ptm_pos-1], protein[ptm_pos]) == false)
...
bool ProteinFragment::has_polymorphism() {
  if(residues[mutval] != protein[mutpos]) return true;
  return false;
}
